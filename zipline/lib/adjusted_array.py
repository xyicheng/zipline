...
NON_CATEGORICAL_DTYPES = BOOL_DTYPES.union(
    FLOAT_DTYPES,
    INT_DTYPES,
    DATETIME_DTYPES,
)

# There are separate dtypes for each possible fixed length of string/unicode,
# so we use kind to check these.
CATEGORICAL_KINDS = frozenset(['O', 'S', 'U'])
...
def can_represent_dtype(dtype):
    ...
    """
    return dtype in NON_CATEGORICAL_DTYPES or dtype.kind in CATEGORICAL_KINDS
...
def _normalize_array(data, missing_value):
    ...
    elif data_dtype.kind in CATEGORICAL_KINDS:
        if not isinstance(missing_value, (str, unicode)):
            raise TypeError(
