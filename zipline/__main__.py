...
def cli(extension, strict_extensions, default_extension):
    ...
    if default_extension:
        default_extension_path = pth.default_extension()
        ...
        extension.insert(0, default_extension_path)
    ...
    for ext in extension:
        try:
            ...
            if strict_extensions:
                # if `strict_extensions` we should raise the actual exception
                # and fail
                raise
            # without `strict_extensions` we should just log the failure
            click.echo(
...
def run(ctx,
    ...
    if not (algotext is not None) ^ (algofile is not None):
        ctx.fail(
